...
API_ID    = os.environ.get("25326160","")
BOT_TOKEN = os.environ.get("6050076233:AAHTfZpIWvzuQfZWA20uBSqYQpzvI1ozoyU","")
CHANNEL_ID = os.environ.get("-1001919117846","")
ADMIN_ID  = int(os.environ.get("587231038","0"))
...
.hdr{background:#1a1a2e;padding:12px 16px;display:flex;justify-content:space-between;align-items:center;border-bottom:2px solid #e63946;}
.badge{background:#e63946;padding:3px 10px;border-radius:4px;font-size:11px;font-weight:bold;animation:blink 1.2s infinite;}
...
if(Hls.isSupported()){var h=new Hls();h.loadSource(url);h.attachMedia(v);h.on(Hls.Events.MANIFEST_PARSED,function(){v.play().catch(function(){});});}
else if(v.canPlayType("application/vnd.apple.mpegurl")){v.src=url;v.play().catch(function(){});}
setInterval(function(){fetch("/update").then(function(r){return r.json();}).then(function(d){document.getElementById("sc").textContent=d.score;document.getElementById("st").textContent=d.status;}).catch(function(){});},30000);
</script>
</body>
</html>"""
...
def update(): return jsonify(MATCH)
...
async def cmd_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        ...
        return
    url = PUBLIC_URL or "URL not set"
    kb = InlineKeyboardMarkup([[
        InlineKeyboardButton("LIVE Dekho", web_app=WebAppInfo(url=url))
    ]])
...
"+MATCH["score"],
    ...
    await update.message.reply_text("Channel par post ho gaya!")
